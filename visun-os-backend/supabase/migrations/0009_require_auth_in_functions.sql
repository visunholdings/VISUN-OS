-- 0009_require_auth_in_functions.sql
-- 0008 revoke EXECUTE của anon trên 4 hàm nhưng thực tế 3 hàm rpc_* vẫn gọi được bằng anon (xác nhận qua
-- curl thật) — Supabase có cơ chế cấp quyền mặc định cho hàm mới không đi qua đường REVOKE thông thường
-- như kỳ vọng. Dữ liệu vẫn an toàn nhờ RLS (0006), nhưng để chắc chắn không phụ thuộc vào việc dò đúng cơ
-- chế GRANT của nền tảng, mỗi hàm tự kiểm tra auth.uid() ngay khi bắt đầu và từ chối rõ ràng nếu chưa đăng nhập.
